...
class Table:
    ...
    __database = None
    __name = None
    __definition = None # only used when creating a table
    def __init__(self, server, database, name, definition=None):
    ...
    def definition(self):
        ...
        if result.len == None or result.len == 0:
            return None
        else:
    ...
    def columns(self):
        ...
        lambdas['items'] = lambda result: dict((x[0], Column.Column(self.__server, self.__database, self, x[0])) for x in result.rows)
...
#         return self.ddl("CHANGE COLUMN `%s` `%s` %s" % (column_name, new_column_name, self.columns[column_name].definition))
# 
#     def change_column(self, column_name, column_definition):
#         return self.ddl("CHANGE COLUMN `%s` `%s` %s" % (column_name, column_name, column_definition))