class Column(object):
    def __init__(self, serverconn, database, table, name):
    ...
    def name(self, name):
        ...
        return self.__serverconn.execute(query)
    ...
    def definition(self, definition):
        query = "ALTER TABLE `%s`.`%s` MODIFY `%s` %s" % (
            self.__database.name, self.__table.name, self.__name, definition)
        return self.__serverconn.execute(query)
